...
for run in ['run-1789867504398998900','run-1789828491134742200']:
    ...
    report['runs'].append({'run':run,'events':status['events'],'sha256':status['sha256'],'asset_id':asset['id'],
        'evaluation_progress':data['evaluation_progress'], 'counts':{p:data[p]['total'] for p in ('evaluations','logs')},
        'evaluation_arrivals':[{'row_id':x['row_id'],'seconds':x['available_at'],'state':x['evaluation_state']} for x in data['evaluations']['items']]})
(root/'artifacts/cd-display-acceptance/cd-display-acceptance-20260920-v1/prepared.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf8')
