...
def main():
    out = ROOT / 'artifacts/human-debug-h0/20260920'
    ...
    try:
        for run in runs:
            ...
            report['runs'].append({'run': run, 'bytes': source.stat().st_size, 'source_sha256_unchanged': before,
                'events': cursor, 'displays': status['displays'], 'index_and_read_seconds': time.perf_counter() - t,
        ...
        with tempfile.TemporaryDirectory(prefix='h0-review-fixture-', dir=out) as td:
            ...
            request = {'review_id': pin['review_id'], 'expected_revision': 0, 'idempotency_key': 'h0-http-test-fixture-0001',
